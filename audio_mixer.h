...
// The following types are acceptable for pre-saturated mixing, as they meet the
// requirement for having a larger range than the saturated mixer result type of
// int16_t. double precision should be preferred on x86/amd64, and single
// precision on ARM. float16 could also work as an input but care must be taken
// to saturate at INT16_MAX-1 and INT16_MIN+1 due to float16 not having a 1:1
// representation of whole numbers in the int16 range.
//
// TODO: set up appropriate compiler defs for mixer presaturate type.

#if (MIXER_PRESATURATE_FLOAT32 + MIXER_PRESATURATE_FLOAT64 +                   \
     MIXER_PRESATURATE_INT32) == 0
